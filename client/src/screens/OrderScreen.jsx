...
const OrderScreen = () => {
	...
	function createOrder(data, actions) {
		...
		return actions.order
			.create({
				//create an actions.order object
				purchase_units: [
					{
			...
			.then((orderId) => {
				// might need to lowecase the D in ID ////////////////////////////////////////////////
				return orderId;
			});
	}
		...
		return actions.order.capture().then(async function (details) {
			try {
				...
				const response = await fetch(
					`${import.meta.env.VITE_APP_CURRENT_SERVER}/api/orders/${
						order._id
					}/pay`,
					{
						method: 'PUT',
	...
	useEffect(() => {
		const fetchOrder = async () => {
			try {
				...
				const response = await fetch(
					`${import.meta.env.VITE_APP_CURRENT_SERVER}/api/orders/${orderId}`,
					{
						method: 'GET',
						headers: {
							'Content-Type': 'application/json',
		...
		if (!order._id || successPay || (order._id && order._id !== orderId)) {
			...
			const loadPaypalScript = async () => {
				...
				try {
					...
					const response = await fetch(
						`${import.meta.env.VITE_APP_CURRENT_SERVER}/api/keys/paypal`,
						{
							method: 'GET',
							headers: {
								'Content-Type': 'application/json',
